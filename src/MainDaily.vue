<template>
    ...
    <div v-else class="container">
        <div v-if="currentUser == null && admingroup != 'Admin_Group'" class="text-center mt-5">
            ...
            <p class="p-permission animated fadeInUp">
                สำหรับสาขาต้องใช้ Account สาขา
            </p>
            <p class="p-permission animated fadeInUp">
                สำหรับสำนักงานใหญ่ต้องมีสิทธิ์ในการเข้าใช้งาน
            </p>
            <p class="p-permission animated fadeInUp">
                หากยังพบปัญหาติดต่อ Service Desk
            </p>
        </div>
        <div v-else>
            <div class="d-flex justify-content-between">
                <div>
                    <h2 class="text-lotus-green">Stock Card daily Report</h2>
                    ...
                    <div class="row">
                        ...
                        <div class="col-3">
                            <div class="form-group">
                                ...
                                <input v-model="formdaily.StartDate" type="date"
                                    class="form-control rounded-pill custom-month-input" />
                            </div>
                        </div>
                        <div class="col-3">
                            <div class="form-group">
                                <label for="end-date-picker">ถึงวันที่</label>
                                <input v-model="formdaily.EndDate" type="date"
                                    class="form-control rounded-pill custom-month-input" />
                            </div>
                        </div>
                        <div class="col text-end">
                            ...
                            <div>
                                <button type="button" class="btn btn-success rounded-pill bg-greens" @click="postDaily">
                ...
                <table class="table table-striped border text-center" ref="example" id="table">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in tableData" :key="index">
                            ...
                            <td class="border">
                                <a :href="'https://colleague.lotuss.com/earchive/api/viewfilefromid/2020/' + item.ID"
                                    target="_blank">{{ getFileName(item.FileRef) }}</a>
                            </td>
                            <td class="border">
...
export default {
    ...
    methods: {
        ...
        async showData() {
            ...
            const url = "https://colleague.lotuss.com/pl-server/api/getStockCardDaily";

            this.test = await axios.get(url, { headers }).then((response) => {
        ...
        async postDaily() {
            ...
            if (this.formdaily.StartDate == "") {
                ...
                this.formdaily.StartDate
                    ? moment(this.formdaily.StartDate).format("DD/MM/YYYY")
                    : "";
            }
            ...
            const requestBody = {
                ...
                StartDate: this.formdaily.StartDate ? moment(this.formdaily.StartDate).format("DD/MM/YYYY") : "",
                EndDate: this.formdaily.EndDate ? moment(`${this.formdaily.EndDate}`).format("DD/MM/YYYY") : "",
            };

            const url =
                "https://colleague.lotuss.com/pl-server/api/getStockCardDaily";
            axios
                ...
                .catch((error) => {
                    ...
                    createToast("เกิดข้อผิดพลาดที่! StockCardDaily", { type: "danger", position: "top-right", showIcon: true, timeout: 5000, });
                    this.initializeDataTable();
                });
        },
